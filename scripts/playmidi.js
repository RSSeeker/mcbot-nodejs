...
function nextChildIndex(bot) {
    ...
    const used = new Set((bot.__playmidiChildren || []).map((c) => c && c.username).filter(Boolean));
    let k = 1;
...
reply('用法: **run playmidi <歌曲.mid> | [参数...] | stop | list（参数无序：1bot|notempo|pitch/auto/inst:N|速度|播放模式）');
    ...
    // 播放中禁止再次启动：防止两个播放并发导致小号同名冲突（duplicate_login）
    if (bot.__playmidiPlaying) {
        reply('正在播放中，请先 **run playmidi stop 再重新播放');
        return;
    }
    ...
    if (!prep) { quitPlaymidiChildren(bot); bot.__playmidiPlaying = false; return; }
