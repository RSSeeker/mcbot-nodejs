...
async function playNBS(bot, session, filePath, reply, log, config, options = {}) {
    ...
    async function prepareSong(file) {
        ...
        if (singleBot) {
            ...
            const alloc = allocateBots(flatNotes, MAX_PACKETS_PER_WINDOW, MAX_BOTS, RATE_BURST_PACKETS);
            botCount = alloc.botCount;
            assign = alloc.assign;
            if (alloc.fits) log('info', `约束分配：${botCount} 个 bot，每 bot 7s 窗口最多 ${alloc.maxLoad7} 包（≤${MAX_PACKETS_PER_WINDOW}）、1s 窗口最多 ${alloc.maxLoad1} 包（≤${RATE_BURST_PACKETS}）`);
    ...
    async function adjustBots(wantedBotCount) {
        ...
        for (let i = session.childBots.length - 1; i >= 0; i--) {
            const c = session.childBots[i] && session.childBots[i].child;
            if (!c || !c._client || c._client.ended) session.childBots.splice(i, 1);
        }
